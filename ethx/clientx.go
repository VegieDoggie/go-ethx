...
func (c *Clientx) UpdateRPCs(newRPCs []string) {
	...
	// newRPCs contains rpc but oldRPCs not: ADD
	...
	// newRPCs not but oldRPCs contains rpc: REMOVE
	for _, rpc := range oldRPCs {
		if !newSet.Contains(rpc) {
			var key *ethclient.Client
